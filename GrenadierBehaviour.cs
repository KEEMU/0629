...
namespace Gamekit3D
{
    ...
    public class GrenadierBehaviour : MonoBehaviour
    {
        ...
        {
            PoolManager.instance.RequestImpact(AttackType.Missile, impactPrefab, pos, Quaternion.identity);
        }
        public void ShootMissiles()
        {
            var missile = PoolManager.instance.RequestProjectile(AttackType.Missile, missilePrefab,muzzle.position,muzzle.rotation);
            if (missile != null)
            {
                missile.GetComponent<Missile>().target = playerTarget;
